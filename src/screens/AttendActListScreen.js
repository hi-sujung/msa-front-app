...
const API_URL = 'http://10.0.2.2:8080/notice/externalact/checked-list';
const AUTO_API_URL = 'http://10.0.2.2:8080/notice/portfolio/create-by-ai';
  ...
  const fetchAttendData = async () => {
    try {
      ...
      if (response.status === 200) {
        setAttendList(response.data); // Set the fetched activity data in the state
      }
    } catch (error) {
    ...
    const createAutoPortfolio = async () => {
      try {
        const response = await axios.post(`${AUTO_API_URL}?memberId=20211021@sungshin.ac.kr&careerField=${careerKeyword}`);
        if (response.status === 200) {
        ...
        >
          <View style={styles.modalContainer}>
            <View style={styles.modalView}>
              <Text style={styles.modalText}>커리어 키워드를 입력해주세요.</Text>
              <TextInput
